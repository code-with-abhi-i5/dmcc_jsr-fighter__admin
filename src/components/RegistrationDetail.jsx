...
export default function RegistrationDetail({ registration, customTeams = [], onClose, onUpdate, onDelete }) {
  ...
  const [editIdValue, setEditIdValue] = useState(registration.registrationId);
  const [idSaving, setIdSaving] = useState(false);

  const handleIdSave = async () => {
    if (!editIdValue.trim() || editIdValue === registration.registrationId) {
      setIsEditingId(false);
      return;
    }
    setIdSaving(true);
    try {
      await updateRegistrationId(registration.id, editIdValue);
      onUpdate(registration.id, { registrationId: editIdValue });
      setIsEditingId(false);
            ...
            {isEditingId ? (
              <div className="flex items-center gap-1 mt-1">
                <input
                  ...
                  className="px-2 py-0.5 text-xs font-mono border border-slate-300 rounded focus:outline-none focus:border-emerald-500 w-32 uppercase"
                  autoFocus
                />
                <button
                  onClick={handleIdSave}
                  disabled={idSaving}
                  className="p-1 bg-emerald-100 text-emerald-700 hover:bg-emerald-200 rounded disabled:opacity-50"
                >
                  {idSaving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
                </button>
                <button
                  onClick={() => {
                    setIsEditingId(false);
                    setEditIdValue(registration.registrationId);
                  }}
              ...
              <div className="flex items-center gap-2 group">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-emerald-500 transition-opacity"
